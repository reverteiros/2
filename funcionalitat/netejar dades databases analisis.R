...
### analisis 3. bitxos - fruits i llavors

TVUFfruitsbitxos <- fruitset %>%
  filter(Species == "TVUF") %>%
  left_join(datapollinatorsall,by=c("Species","Plot"))%>%
...
TVUFseedsbitxos <- TVUFfruitsbitxos %>%
  filter(Fruits == 1) 

TVUHfruitsbitxos <- fruitset %>%
  filter(Species == "TVUH") %>%
  left_join(datapollinatorsall,by=c("Species","Plot"))%>%
  left_join(proporciomorfs,by=c("Plot"))%>%
  mutate(logPollinator_richness = log(Pollinator_richness)) %>%
...
TVUHseedsbitxos <- TVUHfruitsbitxos %>%
  filter(Fruits == 1) 


### analisis 4. polen - llavors

TVUFpollenperplanta <- TVUFpollen %>%
  group_by(Plot, Plant, Species) %>% 
  summarise(Mean_Homospecific=mean(Homospecific),Mean_Heterospecific=mean(Heterospecific),Mean_Total=mean(Total))
