...
fruits <- droplevels(dplyr::filter(seedsraw, !is.na(Avorted) & Total == 4)) %>% 
  ...
  summarise(Samples_seeds=n(),Avorted=mean(Avorted),Pollinated_ovules=(mean(Pollinated)),Seed_set=mean(Seed))

hist(fruits$Samples_seeds)

############# JOIN ALL DATASETS

datafunctionality <- pollen %>%
  dplyr::left_join(., seedweightandviability, by = c("Species","Plot")) %>%
  dplyr::left_join(., fruits, by = c("Species","Plot")) 
